...
namespace Simulator
{
    internal class MenuSystem
    {
        ...
        private static MenuContext SimulationContext = new();
        ...
        public MenuSystem()
        {
            ...
            //List<Menu<Pokemon>> pokemonMenuList = [];
            ...
            //            new MenuListItem<string>(0, "Go back to Pokemon list")
            //        };
            ...
            List<Menu> pokemonMenuList = [];
            for (var i = 0; i < pokemonList.Count; i++)
            {

                var pokemon = pokemonList[i];
                var command = i + 1;
                void renderPokemonInfo()
                {
                    ConsoleUI.WriteLine($"{pokemon.Name} is a level {pokemon.Level} {pokemon.Type} Pokemon.");
                    ConsoleUI.WriteLine($"\nWhat would you like to do with {pokemon.Name}?\n");
                }

                List<Menu> pokemonInfoOptions = [];
                void attacksDisplayCallback()
                {
                    foreach (var attack in pokemon.Attacks)
                    {
                        ConsoleUI.Write($"\n\t{pokemon.Name} knows ");
                ...
                pokemonInfoOptions.Add(new Menu(1, "Show Attacks", attacksDisplayCallback));
                pokemonInfoOptions.Add(new Menu(2, "Evolve", () => {
                    pokemon.Evolve();
                    ConsoleUI.WriteLine("\n\tPress any key to continue");
                    ConsoleUI.ReadKey(intercept: true);
                }));
                
                var pokemonMenu = new Menu(
                    command, 
                    $"{pokemon.Name} info", 
                    pokemonInfoOptions, 
                    renderPokemonInfo
                );
                pokemonMenuList.Add(pokemonMenu);
            }
            
            _simulationMenu = new Menu(
                command: 0, 
                name: "Pokemon Simulator", 
                subMenus: pokemonMenuList,
                introDisplayCallback:renderSimulationIntroUI
            );
        }
